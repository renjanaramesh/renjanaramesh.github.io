...
if (isset($_POST['editSubmit'])) {
  ...
  $sql = "UPDATE library SET description='$editName' WHERE id='$editId' and uid='$user_id'";
...
if (isset($_POST['editDelete'])) {
  ...
  // Update the row in the database
  $sql = "DELETE FROM library WHERE id='$editId' and uid='$user_id'";
  if (mysqli_query($conn, $sql)) {
    echo "Record updated successfully.";
  } else {
    echo "Error updating record: " . mysqli_error($conn);
...
$sql = "SELECT * FROM library WHERE uid='$user_id'";
...
<head>
  ...
  <script>
    function zoomImage(elementId) {
      var zoomedElement = document.getElementById(elementId);
      zoomedElement.classList.toggle("zoomed-image");
    }
  </script>
  <style>
...
<body>
  ...
  <div class="pagetitle">
    ...
    while ($row = mysqli_fetch_assoc($result)): ?>
      <tr>
        ...
        <td>
          <?php
            $fileExtension = pathinfo($row['document_path'], PATHINFO_EXTENSION);
            if ($fileExtension === 'jpg' || $fileExtension === 'png') {
              echo '<a href="' . $row['document_path'] . '" download>Download</a>';
            } elseif ($fileExtension === 'pdf') {
              echo '<a href="' . $row['document_path'] . '" target="_blank">View PDF</a>';
            } else {
              echo '<a href="' . $row['document_path'] . '" download>Download</a>';
            }
          ?>
  ...
  endwhile; ?>
              ...
              <button class="btn btn-primary" type="button" id="" onclick="window.location.href = 'newlibrary.php';" >Add New </button>
    </div>
  ...
  <div id="editForm" class="edit-form">
    ...
    <form method="POST" action="">
      ...
      <label for="editName">New Description:</label>
      <br>
